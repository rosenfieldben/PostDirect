...
const {
  sha256Hex, addressHash, blobStore, readBlob, auditReadStats, auditVerifyChain, auditAppend,
} = require('./store');
...
async function buildProofPackage(dir, letterId, deps) {
  ...
  const exportEvent = { type: 'proof.export', letterId, packageSha256, pdfSha256, complete: missing.length === 0, hasLocalRecord: !!create, fetched, missing: missing.map((m) => m.name), auditCorruptLines };
