...
// accessEnabled is the Access-enforcement flag, injected by the caller (the
// limiter lives here, the perimeter config lives in server.js). Behind Cloudflare
// plus Railway there are TWO proxies and leftmost X-Forwarded-For is
// client-influenced, so it is a weak identity to rate-limit on. But when the
// perimeter is enforced, every request that reaches this limiter has already
// passed the edge (an origin-direct request is refused before routing ever runs),
// and Cloudflare sets CF-Connecting-IP authoritatively, so it is the trustworthy
// client IP. Prefer it only under enforcement; unenforced deployments keep the
// existing PD_TRUST_PROXY / socket behavior byte-for-byte.
function clientIp(req, accessEnabled) {
  if (accessEnabled) {
    const cf = req.headers['cf-connecting-ip'];
    if (cf) { const v = String(Array.isArray(cf) ? cf[0] : cf).trim(); if (v) return ipBucket(v); }
  }
  if (TRUST_PROXY) {
