...
const { accessConfigFromEnv } = require('./access');
...
// Cap on a STREAMED (non-audited) proxy response, so a compromised or runaway
// upstream cannot stream unbounded bytes through to the browser. 52 MB matches
// the request-body ceiling and dwarfs any real letter list/get JSON. Overridable
// via PD_PROXY_STREAM_MAX_BYTES for integration tests (operator-only, same trust
// level as PD_LOB_UPSTREAM).
const PROXY_STREAM_MAX_BYTES = (() => {
  const raw = (process.env.PD_PROXY_STREAM_MAX_BYTES || '').trim();
  return (/^[0-9]+$/.test(raw) && +raw > 0) ? +raw : 52 * 1024 * 1024;
})();
...
function validateStartupConfig(env) {
  ...
  // The Access perimeter is all-or-nothing: exactly one of the two vars set is a
  // half-configured perimeter, which is worse than none (it reads as protected
  // while enforcing nothing), so it dies at boot like the credential floors above.
  // This floor applies even in demo mode: a half-configured perimeter is a
  // mistake regardless of how the server is being run.
  const accessCfg = accessConfigFromEnv(env);
  if (accessCfg.error) errors.push(accessCfg.error);
  return {
    ok: errors.length === 0,
    errors,
    insecureDemo,
...
module.exports = {
  ...
  LOGIN_BODY_LIMIT, PROXY_BODY_LIMIT, PROXY_TIMEOUT_MS, PROXY_STREAM_MAX_BYTES,
