...
const { proxyAuditType, captureProxyEvent, writeSendIntent, DATA_DIR } = require('./store');
...
async function handleProxy(req, res, pathname, search, deps) {
  ...
  // Write-ahead intent for the MUTATING calls (create, cancel): durably record
  // that we are about to send BEFORE the request reaches Lob, and FAIL CLOSED if
  // that record cannot be written. The point is that a send which happened at Lob
  // can never lack a local trace: if we crash, time out, or lose the response
  // after this point, the intent survives with no matching outcome and surfaces
  // in the reconciliation worklist. address.verify is a read, not a mutation, so
  // it is not write-ahead logged. writeSendIntent also writes the request-body
  // blob (the single place it is stored now), so a store failure here refuses the
  // send rather than contacting Lob with no durable record of having done so.
  let intentId = null;
  if (auditType === 'letter.create' || auditType === 'letter.cancel') {
    try {
      intentId = writeSendIntent(DATA_DIR, { lobPath, reqHeaders: req.headers, reqBuf: bodyBuf }, Date.now());
    } catch (e) {
      console.error('Write-ahead intent failed for ' + auditType + '; refusing the send:', e && (e.code || e.message));
      return sendJSON(res, 503, { error: { message: 'Send refused: the durable record could not be written' } });
    }
  }
  ...
  const proxy = LOB_UPSTREAM.transport.request(options, (lobRes) => {
    ...
    if (auditType) {
      ...
      lobRes.on('end', () => {
        ...
        try {
          captureProxyEvent(DATA_DIR, auditType, lobPath, req.headers, upstreamAuth, bodyBuf, lobRes.statusCode, respBuf, intentId);
        } catch (e) {
