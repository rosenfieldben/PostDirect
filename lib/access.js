...
// The default JWKS fetcher: a bounded HTTPS GET returning the parsed JWKS, or a
// rejected promise. Injected in tests. Bounds: a 5s timeout and a 1 MB ceiling,
// so a hung or hostile endpoint cannot stall or exhaust the origin.
function defaultFetchJwks(certsUrl) {
  return new Promise((resolve, reject) => {
    const req = https.get(certsUrl, { timeout: 5000 }, (res) => {
