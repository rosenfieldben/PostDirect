...
function ensureDataDir(dir) {
  try {
    ...
    // mkdirSync's mode only takes effect when the directory is CREATED, and does
    // nothing to one that already exists (the Docker image pre-creates /app/data,
    // and a bind mount or restored backup may arrive with any mode). The store
    // holds client PII and the exact documents mailed, so the 0700 guarantee is
    // enforced at startup here, not assumed from creation.
    fs.chmodSync(dir, 0o700);
    fs.chmodSync(blobs, 0o700);
