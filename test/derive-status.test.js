...
test('tracking-event regex ladder maps to the right {label, variant}', () => {
  ...
  assert.deepStrictEqual(deriveStatus({ tracking_events: [ev('Package processed for delivery')] }), { label: 'Out for delivery', variant: 'progress' });
...
test('date-based fallback when there are no tracking events', () => {
  ...
  assert.deepStrictEqual(deriveStatus({ date_sent: new Date(now() - 3600 * 1000).toISOString() }), { label: 'Processing', variant: 'progress' });
  assert.deepStrictEqual(deriveStatus({ date_sent: new Date(now() - 5 * DAY).toISOString(), expected_delivery_date: new Date(now() - DAY).toISOString() }), { label: 'Delivered (est.)', variant: 'success' });
  assert.deepStrictEqual(deriveStatus({ date_sent: new Date(now() - 5 * DAY).toISOString() }), { label: 'In transit', variant: 'progress' });
});
